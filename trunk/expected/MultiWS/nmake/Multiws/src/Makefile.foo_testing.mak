...
!MESSAGE
!MESSAGE NMAKE /f "Makefile.foo_testing.mak" CFG="Win32 Debug"
!MESSAGE
...
ALL : DEPENDCHECK $(GENERATED_DIRTY) ".\food.dll"
...
!ELSE
	$(DEPGEN) -D_DEBUG -DWIN32 -D_WINDOWS -f "Makefile.foo_testing.dep" "foo.cpp"
!ENDIF
...
".\food.dll" : "$(OUTDIR)" $(DEF_FILE) $(LINK32_OBJS)
    $(LINK32) @<<
...
ALL : DEPENDCHECK $(GENERATED_DIRTY) ".\foo.dll"
...
!IF "$(DEPGEN)" == ""
	...
	@echo Then set the DEPGEN_ROOT environment variable to the full path of Utils.
!ELSE
	$(DEPGEN) -DNDEBUG -DWIN32 -D_WINDOWS -f "Makefile.foo_testing.dep" "foo.cpp"
!ENDIF
...
".\foo.dll" : "$(OUTDIR)" $(DEF_FILE) $(LINK32_OBJS)
    $(LINK32) @<<
  $(LINK32_FLAGS) $(LINK32_OBJS)
<<
...
ALL : DEPENDCHECK $(GENERATED_DIRTY) "$(OUTDIR)\foosd.lib"
...
"$(OUTDIR)\foosd.lib" : "$(OUTDIR)" $(DEF_FILE) $(LINK32_OBJS)
    $(LINK32) @<<
  $(LINK32_FLAGS) $(LINK32_OBJS)
<<
...
ALL : DEPENDCHECK $(GENERATED_DIRTY) "$(OUTDIR)\foos.lib"
...
"$(OUTDIR)\foos.lib" : "$(OUTDIR)" $(DEF_FILE) $(LINK32_OBJS)
    $(LINK32) @<<
  $(LINK32_FLAGS) $(LINK32_OBJS)
<<
...
!IF EXISTS("Makefile.foo_testing.dep")
!INCLUDE "Makefile.foo_testing.dep"
!ENDIF
!ENDIF
...
!IF "$(NO_EXTERNAL_DEPS)" != "1"
!IF EXISTS("Makefile.foo_testing.dep")
	@echo Using "Makefile.foo_testing.dep"
!ELSE
	@echo Warning: cannot find "Makefile.foo_testing.dep"
!ENDIF
!ENDIF
