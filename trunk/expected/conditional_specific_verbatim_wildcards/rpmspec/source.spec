...
%prep
%setup -n source->>ERROR: no value for rpm_version<<

%build

rm -rf $RPM_BUILD_ROOT

mwc.pl -type gnuace -base install -value_project libpaths+=/tmp/mpcrpm/inst/lib -value_project includes+=/tmp/mpcrpm/inst/include  source.mwc
make 
...
mkdir -p $staging_dir
export pkg_dir=$RPM_BUILD_ROOT/source_dir
...
find ${pkg_dir} ! -type d | sed s^${pkg_dir}^^ | sed /^\s*$/d > %{_tmppath}/source.flist
find ${pkg_dir} -type d | sed s^${pkg_dir}^^ | sed '\&^/usr$&d;\&^/usr/share/man&d;\&^/usr/games$&d;\&^/lib$&d;\&^/etc$&d;\&^/boot$&d;\&^/usr/bin$&d;\&^/usr/lib$&d;\&^/usr/share$&d;\&^/var$&d;\&^/var/lib$&d;\&^/var/spool$&d;\&^/var/cache$&d;\&^/var/lock$&d;\&^/tmp/apkg&d' | sed /^\s*$/d | sed 's&^&%dir &' >> %{_tmppath}/source.flist
...
%changelog
* Tue Dec 21 2010 14:10:13 This file was generated by MPC.
  $Id$
  Any changes made directly to this file will
  be lost the next time it is generated.
  MPC Command:
  /tao_builds/mitza/1.5a/ACE_wrappers/bin/mwc.pl -include config -type rpmspec conditional_specific_verbatim_wildcards.mwc
