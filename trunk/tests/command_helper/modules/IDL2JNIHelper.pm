...
sub get_output {
  ...
  my $data = $self->parse($file, \@include, \%macros, \%mparams);
...
sub get_filenames {
  ...
  push(@filenames, $dir . '_' . $name . $local . $ext) if ($bits & 0x20);
...
sub parse {
  ...
  my $str = $self->preprocess($file, $includes, $macros, $mparams);
          ...
          if ($pline =~ /^if\s+(.*)/) {
            ...
            if ($pline =~ /^include\s+(["<])(.*)([>"])$/) {
              ...
              if ($arg =~ /^DCPS_DATA_TYPE\s+"(.*)"$/) {
                ...
                my $dtype = $1;
                $dtype =~ s/.*:://;
                $line = "local interface ${dtype}TypeSupport {}; " .
